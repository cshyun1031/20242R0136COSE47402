...
optimizer = optim.Adam(efficientnet_model.classifier.parameters(), lr=1e-4)
...
for epoch in range(EPOCHS):
    ...
    # Training 단계
    train_loss = train(efficientnet_model, train_loader, optimizer, criterion, device)
    
    # Training Accuracy 계산
    train_acc = calculate_accuracy(efficientnet_model, train_loader, device)
    train_accuracies.append(train_acc)
    
    # Validation Accuracy 계산
    val_acc = calculate_accuracy(efficientnet_model, test_loader, device)
    val_accuracies.append(val_acc)
    
    print(f"Loss: {train_loss:.4f}, Train Accuracy: {train_acc * 100:.2f}%, Validation Accuracy: {val_acc * 100:.2f}%")

# 최종 테스트 정확도 출력
final_accuracy = evaluate(efficientnet_model, test_loader, device)
